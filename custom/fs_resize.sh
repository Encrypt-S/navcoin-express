...
resize_p2() {
        ...
        if [ "$DISTRO_VERSION" = "15.04" ] || [ "$DISTRO_VERSION" = "15.10" ]; then
                p2_finish=$((`fdisk -l /dev/mmcblk1 | grep Disk | grep sectors | awk '{printf $7}'` - 1024))
        else
                p2_finish=$((`fdisk -l /dev/mmcblk1 | grep total | grep sectors | awk '{printf $8}'` - 1024))
        fi

        fdisk /dev/mmcblk1 <<EOF &>> $rsflog
p
d
2
n
